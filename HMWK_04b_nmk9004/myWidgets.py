...
#-----------------------------------------------------------------------------------------------------------------------
  def toolbar_resolution_callback (self) :
    f = simpledialog.askinteger('Resolution Amount', 'Enter Resolution',
                                     initialvalue = self.master.m_Resolution, minvalue = 4)
    if (f is None):
      output_message = f'[Resolution was cancelled, remains {self.master.m_Resolution}]'
      self.master.statusBar_frame.set(output_message)

    else:
      self.master.m_Resolution = f
      output_message = f' Resolution is now {f}'
      self.master.statusBar_frame.set(output_message)
#-----------------------------------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------------------------------
  def toolbar_draw_callback(self) :
    ...
    if  self.master.m_DataSet is None:
      ...
      self.master.ob_world.create_graphic_objects(self.master.ob_canvas_frame.canvas,self.master.m_Resolution,
                                                  self.master.m_DataSet,
